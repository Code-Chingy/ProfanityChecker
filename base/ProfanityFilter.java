...
public class ProfanityFilter {
    ...
    private void init(List<String> wordsList, List<String> phrasesList) {
        ...
        for (String word : wordsList) {
            if (!word.trim().isEmpty()) {
                ...
                this.wordsMap.computeIfAbsent(i, k -> new ArrayList<>());
        ...
        for (String word : phrasesList) {
            if (!word.trim().isEmpty()) {
                ...
                this.phrasesMap.computeIfAbsent(i, k -> new ArrayList<>());
    ...
    public List<String> getBadPhrasesList() {
        return phrasesList;
    }
    ...
    public String getMatchForWord(String word, boolean strictMatch) {
        if (strictMatch) {
            return getBadWordsList().contains(word) ? word : null;
        } else {
            String p = prepareWord(word);
            Character i;
            ...
            for (String badWord : badWordsList) {
                ...
                if (word.equals(badWord) || ((word.contains(badWord) || badWord.contains(word)) && word.length() > 3
                        && DoubleMetaphone.stringMatchScore(word, badWord) > 0.5f)) {
                    return badWord;
