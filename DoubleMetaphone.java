...
import static com.techupstudio.utils.GeneralUtils.Funcs.toFloat;
...
public class DoubleMetaphone{
	...
	public static float stringMatchScore(String word, String matchedWord){

	    if (word == null || matchedWord == null)
	        return 0;
		...
		if (word.length() > matchedWord.length()){
			//mhern - men
			String temp = word;
			for (Character i: matchedWord.toCharArray()){
				temp = temp.replaceFirst(i.toString(), "");
			}
			return toFloat(word.length() - temp.length()) / toFloat(word.length());
		}
		else{
			//men - mhern
			String temp = matchedWord;
			for (Character i: word.toCharArray()){
				temp = temp.replaceFirst(i.toString(), "");
			}
			return toFloat(matchedWord.length() - temp.length()) / toFloat(matchedWord.length());
		}
	}
